...
const milestones = [
  { id: 0, title: 'Design Mockups', amount: '500 USDC', status: 'Approved', submittedAt: '2025-03-01' },
  { id: 1, title: 'Development', amount: '1000 USDC', status: 'Submitted', submittedAt: '2025-03-10' },
...
describe('MilestoneList', () => {
  it('renders empty state for client when no milestones', () => {
    render(<MilestoneList milestones={[]} role="client" onApprove={jest.fn()} onReject={jest.fn()} onSubmit={jest.fn()} />);
  ...
  it('renders empty state for freelancer when no milestones', () => {
    render(<MilestoneList milestones={[]} role="freelancer" onApprove={jest.fn()} onReject={jest.fn()} onSubmit={jest.fn()} />);
    expect(screen.getByText(/client has not added/i)).toBeInTheDocument();
  });

  it('renders all milestones', () => {
    render(<MilestoneList milestones={milestones} role="observer" onApprove={jest.fn()} onReject={jest.fn()} onSubmit={jest.fn()} />);
